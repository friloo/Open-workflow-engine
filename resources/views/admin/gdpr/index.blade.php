<x-app-layout>
    ...
    <x-slot name="subheader">Auskunft (Art. 15) und Vergessenwerden (Art. 17) — Prüfer-fähiger Audit-Trail garantiert.</x-slot>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <x-card title="Auskunft (Art. 15 DSGVO)"
            ...
            <p class="mt-3 text-xs text-slate-500">
                Im ZIP: <code>export.json</code> mit User-Profil, Workflow-Vorgängen,
                Schritt-Historie, Kommentaren, Anhang-Metadata, Audit-Log-Einträgen,
                Saved-Searches und Notification-Präferenzen.
        ...
        <x-card title="Vergessenwerden (Art. 17 DSGVO)"
            ...
            <form method="POST" action="{{ route('admin.gdpr.anonymize') }}" class="space-y-3"
                  onsubmit="return confirm('Diesen User wirklich ANONYMISIEREN? Aktion ist nicht rückgängig zu machen.')">
                @csrf
                <div>
                ...
                <div>
                    <x-input-label for="anon_reason" value="Grund (Begründung der Anonymisierung)" />
                    <textarea id="anon_reason" name="reason" rows="3" required maxlength="500"
                        class="block w-full rounded-lg border-slate-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                        placeholder="z. B. 'Löschungs-Antrag der Person vom 19.05.2026'"></textarea>
                </div>
                <div>
                    <x-input-label for="anon_confirm" value="Sicherheits-Bestätigung" />
            ...
            <div class="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                ...
                <ul class="mt-1 list-disc pl-5 space-y-0.5">
                    ...
                    <li>Passwort wird auf einen Random-Hash gesetzt (Login nicht mehr möglich)</li>
                    <li>Rollen, Custom-Fields, Supervisor-Link werden gelöscht</li>
                    <li>2FA, API-Tokens, Sessions, Saved-Searches, Notification-Prefs werden gelöscht</li>
                    <li>Workflow-Historie + hochgeladene Belege bleiben (begründete Aufbewahrung)</li>
